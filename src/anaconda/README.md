...
```yaml
id: "anaconda-client"
name: "Uanaconda Plugin"
description: "Uanaconda плагин для Yapay SDK"
...
./plugin-debug -plugin anaconda -config config.yaml

# Валидация плагина
./plugin-debug -plugin anaconda -config config.yaml -test validate

# Симуляция платежа
./plugin-debug -plugin anaconda -config config.yaml -test simulate -verbose

# Бенчмарк производительности
./plugin-debug -plugin anaconda -config config.yaml -test benchmark
```
...
```bash
$ ./plugin-debug -plugin anaconda -test validate -verbose

Loading plugin: anaconda
Loading plugin from: plugins/anaconda/anaconda.so
