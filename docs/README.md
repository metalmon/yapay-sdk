...
- Приоритеты версий
- Фичи backend proxy (1C:Фреш, МодульКасса, Wazzup)
- Переход к gRPC плагинам (v2.0)
...
- [Modules-only Builds](architecture/modules-only-builds.md) - Архитектура сборки с shared module cache
...
- `security.request_enforcement` - политика валидации запросов
- `security.rate_limit` - лимит запросов в минуту
- `security.cors.origins` - список разрешенных доменов

**Опциональные поля:**
- `description` - описание плагина
- `yandex.sandbox_mode` - режим песочницы
- `metadata` - дополнительные метаданные
- `notifications` - настройки уведомлений
- `field_labels` - метки для полей в уведомлениях
- `plugin.type` - тип плагина (`builtin`, `so`, `grpc`) - **обязательно для .so плагинов**
- `plugin.path` - путь до .so файла - **обязательно для .so плагинов**
...
### Modules-only сборка
С версии 1.0.10 SDK использует modules-only сборку с shared module cache:
- Каждый плагин имеет собственный `go.mod` файл
- Используется `replace` директива для синхронизации версий зависимостей
- Обеспечивается ABI совместимость между сервером и плагинами
- Никаких зависимостей не скачивается во время сборки плагинов
